...
export class ConversationsComponent implements OnInit, OnDestroy {
  ...
  private listenToGetOneByPublicId(): void {
    this.getOneByPublicIdSub =
      this.conversationService.getOneByPublicId.subscribe(
        (conversationState) => {
          if (conversationState.status === 'OK' && conversationState.value) {
            this.conversations.push(conversationState.value);
          } else {
            this.toastService.show(
              'Error occurred when fetching conversation',
              'DANGER'
            );
          }
        }
  ...
  private listenToConversationCreated(): void {
    this.createSub = this.conversationService.create.subscribe(
      (newConversationState) => {
        if (
          newConversationState.status === 'OK' &&
          newConversationState.value
        ) {
          this.conversations.push(newConversationState.value);
          this.conversationService.navigateToNewConversation(
            newConversationState.value
          );
        } else {
          this.toastService.show(
            'Error occurred when conversation creation',
            'DANGER'
          );
        }
      }
  ...
  private listenToNavigateToConversation(): void {
    this.createOrLoadConversation =
      this.conversationService.createOrLoadConversation.subscribe(
        (userPublicId) => {
          const existingConversation = this.conversations.find(
            (conversation) =>
              conversation.members.findIndex(
                (member) => member.publicId === userPublicId
              ) !== -1
          );
          if (existingConversation) {
            this.conversationService.handleMarkAsRead(
  ...
  private listenToSSENewMessage(): void {
    this.sseService.receiveNewMessage.subscribe(newMessage => {
      const indexToUpdate = this.conversations.findIndex(conversation => conversation.publicId);
