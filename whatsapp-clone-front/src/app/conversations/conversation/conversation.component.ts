...
@Component({
  ...
  standalone: true,
  imports: [FaIconComponent, NgbDropdown, NgbDropdownItem, NgbDropdownMenu, NgbDropdownToggle, SendStateDisplayComponent],
...
export class ConversationComponent {
  ...
  private getReceiverMember() {
    effect(() => {
      this.contact = this.conversationService.getReceiverMember(
        this.conversation()
      );
    });
  }
  ...
  hasUnreadMessage(): boolean {
    if (this.conversation().messages) {
      const unreadMessages = this.conversation().messages.filter(
        (message) =>
