...
export class ConversationService {
  ...
  constructor() {
    effect(() => {
      if (
        this.oauth2Service.fetchUser().status === 'OK' &&
  ...
  handleGetAll(pagination: Pagination) {
    ...
    this.http
      .get<Array<Conversation>>(`${environment.API_URL}/conversations`, {
        params,
      })
      .subscribe({
        next: (conversations) => {
          ...
          this.getAll$.next(
            State.Builder<Array<Conversation>>().forSuccess(conversations)
          );
        },
        error: (err) =>
          this.getAll$.next(State.Builder<Array<Conversation>>().forError(err)),
      });
  }
  ...
  private mapDateToDayJs(conversation: Conversation) {
    if (conversation.messages) {
      conversation.messages.forEach(
        (message) => (message.sendDate = dayjs(message.sendDate))
      );
    }
  ...
  sortConversationByLastMessage(conversations: Array<Conversation>) {
    conversations.sort((conversationA, conversationB) => {
      if (
        ...
        conversationA.messages.length != 0
      ) {
        return (
          conversationB.messages[conversationB.messages.length - 1]
            .sendDate!.toDate()
            .getTime() -
          conversationA.messages[conversationA.messages.length - 1]
            .sendDate!.toDate()
            .getTime()
        );
      } else {
  ...
  private sortMessageByTime(conversations: Array<Conversation>) {
    conversations.forEach((conversation) => {
      conversation.messages.sort((messageA, messageB) => {
        return (
          messageA.sendDate!.toDate().getTime() -
          messageB.sendDate!.toDate().getTime()
        );
      });
    });
  }
    ...
    this.http
      ...
      .subscribe({
        next: (conversation) => {
          ...
          this.create$.next(
            State.Builder<Conversation>().forSuccess(conversation)
          );
        },
        error: (err) =>
          this.create$.next(State.Builder<Conversation>().forError(err)),
      });
  }
  ...
  handleDelete(conversationPublicId: string) {
    ...
    this.http
      .delete<string>(`${environment.API_URL}/conversations`, { params })
      .subscribe({
        next: (publicId) =>
          this.delete$.next(State.Builder<string>().forSuccess(publicId)),
        error: (err) =>
          this.delete$.next(State.Builder<string>().forError(err)),
      });
  }
  ...
  handleGetOne(conversationId: string) {
    ...
    this.http
      .get<Conversation>(
        `${environment.API_URL}/conversations/get-one-by-public-id`,
        { params }
      )
      .subscribe({
        next: (conversation) => {
          this.mapDateToDayJs(conversation);
          this.getOneByPublicId$.next(
            State.Builder<Conversation>().forSuccess(conversation)
          );
        },
        error: (err) =>
          this.getOneByPublicId$.next(
            State.Builder<Conversation>().forError(err)
          ),
      });
  }
  ...
  getReceiverMember(conversation: Conversation): BaseUser {
    return conversation.members.find(
      (member) => member.publicId !== this.connectedUser?.publicId
    )!;
  }
  ...
  handleMarkAsRead(conversationId: string): void {
    ...
    this.http
      .post<number>(
        `${environment.API_URL}/conversations/mark-as-read`,
        {},
        { params }
      )
      .subscribe({
        next: (nbUpdatedMessages) => {
          ...
          this.markAsViewed$.next(
            State.Builder<MessageMarkAsViewedResponse>().forSuccess(
              messagesMarkAsReadResult
            )
          );
        },
        error: (err) =>
          this.markAsViewed$.next(
            State.Builder<MessageMarkAsViewedResponse>().forError(err)
          )
      });
  }
}
