...
export class Oauth2AuthService {
  ...
  private fetchUser$: WritableSignal<State<ConnectedUser>> = signal(
    State.Builder<ConnectedUser>().forSuccess({ email: this.notConnected })
  );
  ...
  fetch(): void {
    this.fetchUserHttp$.subscribe({
      next: (user) =>
        this.fetchUser$.set(State.Builder<ConnectedUser>().forSuccess(user)),
